...
#      replacement method: current version of replace_metadata() is
#      a poor solution since it forces writing two times.
...
class RstFile:
    ...
    def get_metadata_value(self, key, default=None):
        """ returns the value corresponding to key or default if not
        present """
        return self.metadata.get(key, default)

    def set_metadata_value(self, key, value):
        """ adds/changes the value of key in metadata """
    ...
    def replace_metadata(self):
        """ replaces the metadata of the file with the current values """
        self.save_without_metadata()
